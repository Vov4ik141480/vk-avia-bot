...
import tickets_api
from config import aviasales_data_api_url, airports_timezone_api_url, AVIASALES_API_TOKEN
...
class FlightData:
    ...
    def make_request_to_api_for_date(self):
        ...
        url_params = [self.depart_city_code, self.arrive_citi_code, self.departure_time, self.api_token]
        while True:
            try:
                request_to_api_for_date = tickets_api.check_request_to_api(api, url_params)
                ...
                if request_to_api_for_date.json()["data"]:
                    try:
                        departure_date = request_to_api_for_date.json()["data"][0]["departure_at"]
    ...
    def make_requests_to_api_for_month(self):
        ...
        while True:
            try:
                request_to_api_for_month = tickets_api.check_request_to_api(api, url_params)
            except NotCriticalExeption as exc:
                self.bot_.send_warning(self.user_id, exc.args[0])
                continue
            except CriticalExeption as exc:
    ...
    def check_new_date(self, departure_date):
        ...
        if arrow.now() < (arrow.get(departure_date) + timedelta(hours=1)):
            ...
            raise DateNotCorrect("На сегодня билетов уже нет. Ищу на затра ...", departure_time.strftime("%Y-%m-%d"))
    ...
    def get_timezones(self, airport_code):
        ...
        while True:
            try:
                request_to_api_for_timezone = tickets_api.check_request_to_api(api, url_param)
            except NotCriticalExeption as exc:
                self.bot_.send_warning(self.user_id, exc.args[0])
                continue
            except CriticalExeption as exc:
    ...
    def api_response_handle(self, request_to_api):
        ...
        данных по билетам
        """
        current_destination_airport = request_to_api.json()["data"][0]["destination_airport"]
        ...
        for ticket_data in request_to_api.json()["data"]:
            ...
            arrive_datetime = self.get_arrive_datetime(departure_date, arrive_timezone, flight_duration)
